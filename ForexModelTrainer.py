...
time_frames = ['1M', '15M', '30M', '1H', '1D']
time = [1,15,30,1,1]
start_dates = [1, 2, 2, 60, 365]
symbols = ['EURUSD', 'GBPJPY', 'GBPUSD', 'USDJPY', 'USDCHF', 'USDCAD', 'AUDUSD', 'NZDUSD', 'XAUUSD']
intervals = [ 'minute', 'minute', 'minute', 'hourly', 'daily']

for pair in symbols:
    for index, time_frame in enumerate(time_frames):
        start_date = (datetime.now(timezone('US/Eastern')) - timedelta(days=start_dates[index])).strftime("%Y-%m-%d")
